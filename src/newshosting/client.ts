...
export type NewshostingOptions = {
  username: string;
  password: string;
  host?: string;
  ip?: string;
...
export type NewshostingSearchResponse = {
...
function parseGroups(xml: string): NewshostingSearchResponse {
  const groups = xml.match(/<groups\b[^>]*>[\s\S]*?<\/groups>/i)?.[0] || "";
  const totalItems = Number.parseInt(attrValue(groups, "items") || "0", 10) || 0;
  const totalPages = Number.parseInt(attrValue(groups, "pages") || "0", 10) || 0;
...
function parseGroupDetail(xml: string): { newsgroups: string[]; files: Array<{ num: string; name: string; timestamp: string }> } {
  const group = xml.match(/<group\b[^>]*>[\s\S]*?<\/group>/i)?.[0] || "";
  const newsgroups = blocks(group, "newsgroup").map(block => block.replace(/<\/?newsgroup[^>]*>/gi, "").trim()).filter(Boolean);
...
function parseFileDetail(xml: string, fallbackAuthor: string): { author: string; articles: NzbFile["articles"] } {
  const file = xml.match(/<file\b[^>]*>[\s\S]*?<\/file>/i)?.[0] || "";
  const author = tagValue(file, "author") || fallbackAuthor;
  const articlesBlock = file.match(/<articles\b[^>]*>[\s\S]*?<\/articles>/i)?.[0] || "";
  const articles = blocks(articlesBlock, "article").map(article => ({
...
function buildNzb(files: NzbFile[], newsgroups: string[]): string {
  const fileXml = files.map((file, fileIndex) => {
    ...
    const groupsXml = newsgroups.map(group => `<group>${xmlEscape(group)}</group>`).join("");
    const segmentsXml = file.articles.map(article =>
...
export class NewshostingClient {
  ...
  private options: Required<NewshostingOptions>;

  constructor(options: NewshostingOptions) {
    this.options = {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onTimeout = () => cleanup(() => reject(new Error("newshosting_connect_timeout")));
  ...
  async createNzb(index: string, scope: string, itemId: string): Promise<string> {
    ...
    return buildNzb(nzbFiles, group.newsgroups);
  }
  ...
  private async send(bodyXml: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      this.socket!.write(encodeFrame(xml), error => error ? reject(error) : resolve());
    });
  }
