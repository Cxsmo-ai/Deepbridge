...
| `DEEPBRID_USENET_FINDER_ENABLED` | `true` | Enables the paired browser-extension Deepbrid Finder source. |
...
Deepbridge can optionally use Deepbrid's premium website Usenet Finder through a paired browser extension. This is separate from the API-key-only official Stremio source and external Newznab indexers.

Load the MV2 extension release in an MV2-capable browser, keep a logged-in Deepbrid Finder tab open, then open the per-configuration pairing URL produced by the dashboard. Oracle sends Finder requests only to that paired extension. No website cookie, copied browser headers, Google credential, or Byparr endpoint is configured in Deepbridge.
